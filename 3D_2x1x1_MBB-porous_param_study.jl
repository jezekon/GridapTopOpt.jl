...
function run_single_optimization(
    ...
    write_dir = "./results/parametric_study_MBB_porous/run$(lpad(run_id, 2, '0'))_γ$(γ)_maxsteps$(max_steps)_α$(α_coeff)/"
...
function save_study_results(results, study_start, start_run, end_run)
    results_dir = "./results/parametric_study_MBB_porous/"
    mkpath(results_dir)
...
function print_final_summary(results, total_time)
    ...
    println("="^60)
    println("Results saved to: ./results/parametric_study_MBB_porous/results_batch_*.csv")
    println(
