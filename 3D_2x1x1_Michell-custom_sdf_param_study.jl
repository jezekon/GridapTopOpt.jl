# ==============================================================================
# PARAMETRIC STUDY: 3D Michell-type Beam Level-Set Optimization
# ==============================================================================
# Full factorial study over: γ, max_steps
# α_coeff = 4 * max_steps * γ  (derived, not independent)
# ==============================================================================
...
# STUDY CONFIGURATION
# ==============================================================================
const TASK_NAME = "3D_2x1x1_Michell_01tol_r2.0"
const LEVELSET_FILE = "data/3D_2x1x1_Michell/$(TASK_NAME)_SDF_B-0.05.jld2"

# Parameter grid
const PARAM_γ = [0.06, 0.08, 0.1]
const PARAM_MAX_STEPS = [1, 2, 3, 4]
...
function run_single_optimization(
    ...
    write_dir = "./results/parametric_study_Michell/$(TASK_NAME)/run$(lpad(run_id, 2, '0'))_γ$(γ)_maxsteps$(max_steps)/"
    ...
    # ==========================================================================
    # FE SPACES - Michell: 4 corner supports clamped (U1=U2=U3=0)
    # ==========================================================================
    ...
    # ==========================================================================
    # HILBERT REGULARIZATION (α_coeff derived from γ and max_steps)
    # ==========================================================================
...
function run_parametric_study(start_run::Int, end_run::Int)
    # Generate all parameter combinations (only γ × max_steps)
    param_combinations =
    ...
    println("="^60)
    println("PARAMETRIC STUDY: 3D Michell Level-Set Optimization")
    println("="^60)
...
function save_study_results(results, study_start, start_run, end_run)
    results_dir = "./results/parametric_study_Michell/$(TASK_NAME)/"
    mkpath(results_dir)
...
function print_final_summary(results, total_time)
    ...
    if !isempty(converged)
        ...
        println(
            "  Run $(best.run_id): γ=$(best.γ), max_steps=$(best.max_steps), α_coeff=$(best.α_coeff)",
        )
    ...
    if !isempty(converged)
        ...
        println("\nFASTEST CONVERGENCE:")
        println(
            "  Run $(fastest.run_id): γ=$(fastest.γ), max_steps=$(fastest.max_steps), α_coeff=$(fastest.α_coeff)",
        )
        println(
...
# ==============================================================================
# Usage:
#   julia script.jl               → all runs (1-8)
#   julia script.jl 3             → run 3 only
#   julia script.jl 3 6           → runs 3-6
#
#   JULIA_DEPOT_PATH="$(pwd)/.julia_depot" julia --project=. 3D_2x1x1_Michell-custom_sdf_param_study.jl 1 3

total = length(PARAM_γ) * length(PARAM_MAX_STEPS)

if length(ARGS) == 0
    run_parametric_study(1, total)
